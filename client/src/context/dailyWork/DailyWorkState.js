...
import DailyWorkContext from "./DailyWorkContext";
...
const DailyWorkState = props => {
  ...
  const [state, dispatch] = useReducer(DailyWorkReducer, initalState);
  ...
  const addCall = async call => {
    ...
    try {
      const res = await axios.post("/api/daily", call, config);
